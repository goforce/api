...
func Call(co commons.Connection, reqp *XE, result interface{}, headers ...*XE) error {
	return Post(co.Server(),
		reqp.write(&bytes.Buffer{}).Bytes(),
		result, append(headers, &XE{"tns:SessionHeader", XE{"tns:sessionId", co.Token()}})...,
	)
}
