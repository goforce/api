...
func Insert(co commons.Connection, records []commons.Record, headers ...*XE) ([]DmlResult, error) {
	start := time.Now()
	...
	for i, re := range result.DmlResults {
		if re.Success {
			records[i].Set("Id", re.Id)
		}
	}
	return result.DmlResults, nil
}

func Update(co commons.Connection, records []commons.Record, headers ...*XE) ([]DmlResult, error) {
	start := time.Now()
...
func Upsert(co commons.Connection, records []commons.Record, externalId string, headers ...*XE) ([]DmlResult, error) {
	start := time.Now()
...
func Delete(co commons.Connection, records []commons.Record, headers ...*XE) ([]DmlResult, error) {
	start := time.Now()
...
func writeRecord(buf *Buffer, record commons.Record, fields []string, fieldsToNull []string) error {
